...
export const getEventColor = (categoryName: string): string => {
    ...
    return category?.color || '#6366F1'; // Default color if category not found
};
...
export const getCurrentMonthDays = (
    ...
    Array.from({ length: daysInMonth }, (_, i) =>
        createDayItem(i + 1, month, year, false, events)
    );
...
export const getNextMonthDays = (
    ...
    return Array.from({ length: totalCells - currentDays }, (_, i) =>
        createDayItem(i + 1, nextMonth, nextMonthYear, true, events)
    );
};
